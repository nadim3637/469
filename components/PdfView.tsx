...
import { generateCustomNotes } from '../services/groq'; // AI Service
...
export const PdfView: React.FC<Props> = ({ 
  ...
  const handleSmartBilingual = async () => {
      if (!activePdf || activePdf.startsWith('http')) return;

      // 1. If already bilingual, just toggle
      if (activePdf.includes('|||')) {
          setActiveLang(prev => prev === 'ENGLISH' ? 'HINDI' : 'ENGLISH');
          return;
      }

      // 2. If English only, confirm generation
      if (!confirm("Hindi translation not found on cloud.\n\nGenerate Smart Bilingual Notes using AI? (Takes ~10s)")) return;

      setIsAiTranslating(true);
      try {
          const prompt = `Translate and expand the following educational content into Hindi (Conversational, Deep Analysis, Gemini Style). 
          Make it at least 1000-1500 words. Use emojis and bullet points for clarity. 
          Keep it engaging like a personal tutor explaining to a student.
          
          Original Content:
          ${activePdf.slice(0, 2000)}... (truncated for context)`; // Send partial context if too long, or better send topic title

          const hindiContent = await generateCustomNotes(chapter.title + " - " + subject.name, prompt, settings?.aiModel);
          
          if (hindiContent) {
              const newContent = activePdf + "\n\n|||\n\n" + hindiContent;
              setActivePdf(newContent);
              setActiveLang('HINDI');
              
              // SAVE PERMANENTLY (Local & Cloud if possible)
              // We need to know which field to update in contentData. 
              // This is tricky because we don't know which field triggered activePdf (Free/Premium).
              // For simplicity, we just save to LocalStorage cache for this session/user.
              
              // TODO: Ideally update the source field in contentData and save to storage
              // For now, we rely on the user having the new content in activePdf state.
              alert("✅ AI Generated Hindi Notes & Saved!");
          }
      } catch (e) {
          console.error(e);
          setAlertConfig({ isOpen: true, message: "AI Generation Failed. Please check internet." });
      } finally {
           ...
           {activePdf && !activePdf.startsWith('http') && (
               <div className="flex items-center mr-2">
                   {activePdf.includes('|||') ? (
                       ...
                       <button 
                           onClick={handleSmartBilingual}
                           disabled={isAiTranslating}
                           className="flex items-center gap-1 bg-gradient-to-r from-orange-100 to-amber-100 text-orange-700 px-3 py-1.5 rounded-full border border-orange-200 shadow-sm animate-in fade-in"
                       >
                           {isAiTranslating ? <Zap size={14} className="animate-spin" /> : <Globe size={14} />}
                           <span className="text-[10px] font-black">{isAiTranslating ? 'Generating...' : 'Translate (AI)'}</span>
                       </button>
                   )}
               </div>
           )}
