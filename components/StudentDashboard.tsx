...
import { speakText, stopSpeech, getAvailableVoices, setPreferredVoice } from '../utils/textToSpeech';
import { storage } from '../utils/storage';
import { HeroSection } from './dashboard/HeroSection';
import { SmartSlide } from './dashboard/SmartSlide';
import { PowerZone } from './dashboard/PowerZone';
...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating, onNavigateToChapter, isDarkMode, onToggleDarkMode }) => {
          ...
          if (data) {
              ...
              if (new Date(latest.timestamp).getTime() > Number(lastRead)) {
                  setHasNewUpdate(true);
                      ...
                      const alertShown = await storage.getItem(alertKey);
                      if (!alertShown) {
  ...
  useEffect(() => {
      storage.getItem(`nst_test_attempts_${user.id}`).then((saved) => {
          if (saved) setTestAttempts(typeof saved === 'string' ? JSON.parse(saved) : saved);
      });
  }, [user.id]);
  ...
  const [voiceGuideEnabled, setVoiceGuideEnabled] = useState(false); // Voice Guide Toggle
  const [voiceScripts, setVoiceScripts] = useState<any>({});
  const [showVoiceSettings, setShowVoiceSettings] = useState(false);
  const [voiceOptions, setVoiceOptions] = useState<SpeechSynthesisVoice[]>([]);

  // Load Voice Data & Settings
  useEffect(() => {
      const loadVoiceData = async () => {
          const scripts = await storage.getItem('nst_voice_scripts');
          if (scripts) setVoiceScripts(scripts);
          
          const savedEnabled = await storage.getItem('nst_voice_guide_enabled');
          if (savedEnabled === 'true') setVoiceGuideEnabled(true);
  ...
  // Auto-Speak Welcome
  useEffect(() => {
      // Only speak on initial mount if enabled, or when toggled ON
      if (voiceGuideEnabled && voiceScripts.WELCOME) {
          // Debounce to prevent multiple speaks
          const timer = setTimeout(() => speakText(voiceScripts.WELCOME), 1500);
          return () => clearTimeout(timer);
      } else if (!voiceGuideEnabled) {
          stopSpeech();
      }
  }, [voiceGuideEnabled]); // Only trigger on toggle or initial load (if enabled)

  const handleVoiceFeedback = (key: string) => {
      if (voiceGuideEnabled && voiceScripts[key]) {
          speakText(voiceScripts[key]);
      }
  };
  ...
  useEffect(() => {
      const loadMorningInsight = async () => {
          ...
          if (now.getHours() >= 10) {
              ...
              const savedBanner = await storage.getItem<any>('nst_morning_banner');
              
              if (savedBanner) {
                  const parsed = typeof savedBanner === 'string' ? JSON.parse(savedBanner) : savedBanner;
              ...
              if (!isGen) {
                  storage.setItem(`nst_insight_gen_${today}`, 'true'); // Lock
                  try {
                      ...
                      await generateMorningInsight(
                          ...
                          settings, 
                          (banner) => {
                              storage.setItem('nst_morning_banner', JSON.stringify(banner));
                      ...
                      storage.removeItem(`nst_insight_gen_${today}`); // Unlock
                  }
              }
          }
      };
  ...
  useEffect(() => {
      const checkAndGenerateChallenges = async () => {
          ...
          const lastDaily = await storage.getItem(`daily_challenge_gen_${dateStr}`);
          if (!lastDaily) {
              // Generate Logic
              if (user && user.classLevel && settings) {
                  try {
                      ...
                      storage.setItem(`daily_challenge_data`, JSON.stringify(challenge));
                      storage.setItem(`daily_challenge_gen_${dateStr}`, 'true');
          ...
          if (today.getDay() === 0) {
              const lastWeekly = await storage.getItem(`weekly_challenge_gen_${dateStr}`);
              if (!lastWeekly) {
                  if (user && user.classLevel && settings) {
                      try {
                          const challenge = await generateDailyChallengeQuestions(
                          ...
                          storage.setItem(`weekly_challenge_data`, JSON.stringify(challenge));
                          storage.setItem(`weekly_challenge_gen_${dateStr}`, 'true');
  ...
  const startAutoChallenge = async (type: 'DAILY' | 'WEEKLY') => {
      ...
      if (stored) {
          const challenge = typeof stored === 'string' ? JSON.parse(stored) : stored;
          const mappedTest: WeeklyTest = {
              id: challenge.id,
              name: challenge.name,
  ...
  const handleAiNotesGeneration = async () => {
      ...
      const currentUsage = parseInt(await storage.getItem(usageKey) || '0');
  ...
  const toggleLayoutVisibility = (sectionId: string) => {
      ...
      storage.setItem('nst_system_settings', JSON.stringify(newSettings));
  ...
  useEffect(() => {
      const checkReward = async () => {
          ...
          const yActivity = parseInt(await storage.getItem(`activity_${user.id}_${yDateStr}`) || '0');
          const yClaimed = await storage.getItem(`reward_claimed_${user.id}_${yDateStr}`);
          ...
          if (!yClaimed && (!user.subscriptionTier || user.subscriptionTier === 'FREE')) {
              ...
              if (reward) {
                  ...
                  storage.setItem(`reward_claimed_${user.id}_${yDateStr}`, 'true');
              }
          }
      };
      checkReward();
  ...
  const renderMainContent = () => {
          ...
          return (
              <div className="space-y-6 pb-24">
                  ...
                  <div className="flex justify-end px-2 gap-2">
                      {voiceGuideEnabled && (
                          <button 
                              ...
                              className="px-3 py-1.5 rounded-full text-xs font-bold bg-white border border-slate-200 text-slate-600 flex items-center gap-1"
                          >
                              <Settings size={12} /> Voice Options
                          </button>
                      )}
                      <button 
                          onClick={() => {
                              ...
                              storage.setItem('nst_voice_guide_enabled', String(newState));
                              if (newState) {
                                  speakText("Voice Guide Enabled. Tap on any button to hear what it does.");
                              } else {
                                  stopSpeech();
                              }
                          }}
                          className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all ${voiceGuideEnabled ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-slate-100 text-slate-500 border border-slate-200'}`}
                      >
                          {voiceGuideEnabled ? <Volume2 size={14} /> : <VolumeX size={14} />}
                  ...
                  <HeroSection 
                      ...
                      onStartStudy={(mins) => showAlert(`Focus Timer set for ${mins} minutes!`, 'SUCCESS')}
                      onAiTutor={() => setShowAiModal(true)}
                      onContinueLearning={() => onTabChange('COURSES')}
                      onDailyChallenge={() => startAutoChallenge('DAILY')}
                      settings={settings}
                  />
                  ...
                  <SmartSlide 
                      settings={settings}
                      onAiTour={() => showAlert("AI Tour Starting...", "INFO")} 
                      onOffers={() => onTabChange('STORE')}
                      onNotice={() => showAlert(settings?.noticeText || "No new notices.", "INFO", "Notice Board")}
                  >
                      <BannerCarousel>
                          ...
                          {morningBanner && (
                              <div className="mx-1 h-48 bg-gradient-to-r from-orange-100 to-amber-100 p-4 rounded-2xl shadow-sm border border-orange-200 overflow-y-auto">
                                  <div className="flex justify-between items-start mb-2">
                                      ...
                                      <SpeakButton text={`${morningBanner.title}. ${morningBanner.wisdom}`} className="text-orange-600 hover:bg-orange-200" iconSize={16} />
                                  </div>
                          ...
                          <DashboardSectionWrapper id="live_challenges" label="Live Challenges">
                              <div className="mx-1 h-48 bg-slate-900 p-4 rounded-2xl shadow-lg text-white border border-slate-700 relative overflow-hidden flex flex-col">
                                  ...
                                  <h3 className="font-black text-white flex items-center gap-2 mb-3 relative z-10">
                                      <Rocket size={18} className="text-indigo-400" /> Challenges
                                  </h3>
                                  <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide relative z-10 flex-1 items-center">
                                      <button onClick={() => startAutoChallenge('DAILY')} className="min-w-[140px] bg-slate-800 p-3 rounded-xl border border-slate-700 hover:border-indigo-500 transition-all text-left group">
                                          <p className="text-[10px] font-bold text-yellow-400 uppercase mb-1">Daily</p>
                                          <p className="font-bold text-sm text-white leading-tight mb-2">Mixed Practice</p>
                                      </button>
                                      {challenges20.map(c => (
                                          <button key={c.id} onClick={() => startChallenge20(c)} className="min-w-[140px] bg-slate-800 p-3 rounded-xl border border-slate-700 hover:border-indigo-500 transition-all text-left">
                                              <p className="text-[10px] font-bold text-indigo-400 uppercase mb-1">Special</p>
      ...
      // LEGACY CODE REMOVAL PLACEHOLDER - TO BE DELETED
      if (false) {
                const isPremium = user.isPremium && user.subscriptionEndDate && new Date(user.subscriptionEndDate) > new Date();
                  ...
                  <div className="bg-gradient-to-r from-pink-50 to-rose-50 p-4 rounded-2xl border border-pink-100 shadow-sm mt-4">
                      ...
                      <div>
                          ...
                          <div className="grid grid-cols-3 gap-3">
                              ...
                              <button onClick={() => { setShowInbox(true); handleVoiceFeedback('INBOX'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform relative group">
                              ...
                              <button onClick={() => { onTabChange('ANALYTICS'); handleVoiceFeedback('ANALYTICS'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              <button onClick={() => { setShowMonthlyReport(true); handleVoiceFeedback('MARKSHEET'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              <button onClick={() => { onTabChange('HISTORY'); handleVoiceFeedback('HISTORY'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              <button onClick={() => { onTabChange('AI_HISTORY'); handleVoiceFeedback('AI_HISTORY'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              <button onClick={() => { onTabChange('STORE'); handleVoiceFeedback('STORE'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              <button onClick={() => { onTabChange('SUB_HISTORY' as any); handleVoiceFeedback('MY_PLAN'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              {isGameEnabled && (
                                ...
                                <button onClick={() => { onTabChange('GAME'); handleVoiceFeedback('GAMES'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              <button onClick={() => { onTabChange('REDEEM'); handleVoiceFeedback('REDEEM'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              <button onClick={() => { onTabChange('PRIZES'); handleVoiceFeedback('PRIZES'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              <button onClick={() => { onTabChange('LEADERBOARD'); handleVoiceFeedback('RANKS'); }} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
      ...
      if (activeTab === 'PROFILE') return (
                <div className="animate-in fade-in zoom-in duration-300 pb-24">
                    ...
                    <div className="space-y-4">
                        ...
                        <button onClick={async () => { await storage.removeItem('nst_current_user'); window.location.reload(); }} className="w-full bg-red-500 text-white py-3 rounded-xl font-bold hover:bg-red-600">🚪 Logout</button>
                    </div>
                </div>
      );
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
                <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
                    <h3 className="text-lg font-bold mb-4 text-slate-800 flex items-center gap-2">
                        <Volume2 size={20} className="text-blue-600" /> Voice Settings
                    </h3>
                    
                    <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
                        <p className="text-xs text-slate-500 mb-2">Select your preferred assistant voice:</p>
                        {voiceOptions.length === 0 && <p className="text-xs text-red-500">No voices detected. Please check your device settings.</p>}
                        {voiceOptions.map((voice) => {
                            const isSelected = localStorage.getItem('nst_preferred_voice_uri') === voice.voiceURI;
                            return (
                                <button
                                    key={voice.voiceURI}
                                    onClick={() => {
                                        setPreferredVoice(voice.voiceURI);
                                        speakText("This is my new voice. How do I sound?", voice);
                                        // Force update to reflect selection UI
                                        setShowVoiceSettings(prev => !prev);
                                        setTimeout(() => setShowVoiceSettings(true), 0);
                                    }}
                                ...
                                >
                                    <span className="text-sm font-bold truncate pr-2 max-w-[70%]">{voice.name}</span>
                                    <span className="text-[10px] bg-slate-100 px-2 py-1 rounded text-slate-500">{voice.lang}</span>
                                </button>
                            );
                        })}
                    </div>
                    ...
                    <div className="mt-4">
                        <button onClick={() => setShowVoiceSettings(false)} className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg">
                            Done
                        </button>
                    </div>
                </div>
