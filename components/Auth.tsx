...
export const Auth: React.FC<Props> = ({ onLogin, logActivity }) => {
  ...
  useEffect(() => {
      storage.getItem<any>('nst_system_settings').then(s => {
          if (s) setSettings(typeof s === 'string' ? JSON.parse(s) : s);
      });

      // Auto-Speak Welcome on Login Screen Load
      const timer = setTimeout(() => {
          speakText("Welcome to Ideal Inspiration Classes. Please login to continue your learning journey.");
      }, 1000);
  ...
  const handleCompleteSignup = async () => {
      const users = await storage.getItem<User[]>('nst_users') || [];
      ...
      try {
          ...
          storage.setItem('nst_users', updatedUsers);
