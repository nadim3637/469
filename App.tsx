...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      storage.setItem('nst_dark_mode', darkMode.toString());
  }, [darkMode]);
  ...
  useEffect(() => {
    const initApp = async () => {
        try {
            ...
            const storedSettings = await storage.getItem<any>('nst_system_settings');
            if (storedSettings) {
                try {
            ...
            // Restore User
            const loggedInUserStr = await storage.getItem<any>('nst_current_user');
            let restoredUser: User | null = null;

            if (loggedInUserStr) {
                try {
                    let user: User = typeof loggedInUserStr === 'string' ? JSON.parse(loggedInUserStr) : loggedInUserStr;
            ...
            if (restoredUser) {
                ...
                if (lastTracker !== today) queue.push('TRACKER');

                const lastChallenge = await storage.getItem<string>('nst_last_daily_challenge_date');
                if (lastChallenge !== today) queue.push('CHALLENGE');
            }

            setPopupQueue(queue);

            if (restoredUser) {
                let initialView = (restoredUser.role === 'ADMIN' || restoredUser.role === 'SUB_ADMIN') ? 'ADMIN_DASHBOARD' : 'STUDENT_DASHBOARD';
                let safeClass = restoredUser.classLevel || null;
                const freeModes = loadedSettings.appMode?.allowedModesForFree || ['SCHOOL'];
                if (!restoredUser.isPremium && safeClass === 'COMPETITION' && !freeModes.includes('COMPETITION')) {
                    safeClass = null;
                ...
                setState(prev => ({ 
                    ...
                    selectedBoard: restoredUser!.board || null, 
                    selectedClass: safeClass, 
                    selectedStream: restoredUser!.stream || null, 
                    language: restoredUser!.board === 'BSEB' ? 'Hindi' : 'English', 
  ...
  if (isInitializing) {
      return (
          <div className="min-h-screen bg-white flex flex-col items-center justify-center">
              ...
              <h2 className="text-xl font-bold text-slate-800">Initializing App...</h2>
              <p className="text-slate-500 text-sm">Upgrading storage engine...</p>
          </div>
      );
  }
